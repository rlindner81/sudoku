...
export default class Solver {
  ...
  constructor(size, seed) {
    ...
    for (let y = 0; y < this.size; y++) {
      for (let x = 0; x < this.size; x++) {
        ...
        let box = this._boxPositions(Math.floor(x / this.width), Math.floor(y / this.height)).filter(i => i !== index);
  ...
  _boxPositions(offsetX, offsetY) {
    let indices = [];
    let size = this.width * this.height;
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        indices.push((x + offsetX * this.width) + (y + offsetY * this.height) * this.size);
      }
    }
    return indices;
  }
  ...
  searchAnySolution(board) {
    ...
    let searchValues = this.randomness.shuffle(board[searchInfo.position].split(""));
  ...
  generateFullGrid() {
    ...
    let baseGrid = this.randomness.shuffle(numbers(1, this.size).map(charFromNum)).join("") + EMPTY_CHAR.repeat(this.cells - this.size);
    let baseBoard = this.boardFromGrid(baseGrid);
    let solution = this.searchAnySolution(baseBoard)
    return this.gridFromBoard(solution);
  }

  hasUniqueSolution(searchInfo, board, fullGrid) {
    if (searchInfo.solved) {
      return true;
    }
  ...
  static _emptyAtPos(grid, pos) {
    return grid.substr(0, pos) + EMPTY_CHAR + grid.substr(pos + 1);
  }

  generateHintGrid(attempts) {
    let fullGrid = this.generateFullGrid();
    let positions = this.randomness.shuffle(numbers(0, this.cells));
    let grid = fullGrid;
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      let newGrid = Solver._emptyAtPos(grid, positions[0]);
      ...
      if (this.hasUniqueSolution(searchInfo, board, fullGrid)) {
        ...
        positions = this.randomness.shuffle(positions);
        attempt++;
        continue;
      }
    }
...
function generateGridsPack(minSize) {
  ...
  for (let size = minSize; size <= 25; size++) {
    let solver = new Solver(size, seed);
    if (solver === null) {
      continue;
    }
