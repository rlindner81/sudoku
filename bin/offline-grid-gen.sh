...
stop_start_script() {
  if [ -f "$script_pid" ]; then kill `cat "$script_dir/$1-$script_name.pid"` >/dev/null 2>&1; fi
  nohup node --experimental-modules "$script_dir"/../src/offline/generate.mjs --size $1 >"$script_txt" 2>&1 &
